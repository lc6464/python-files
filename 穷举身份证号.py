...
def GetStudentData(id18: Union[str, list]):
	def Get(id18: str):
		res = urlopen('http://www.lcyzczb.com/cjcx/edut_student_1.asp?id=g%s' % id18)
		result = res.read().decode('gb18030', 'ignore')
		if result.find('<script>alert("不存在该ID的学生");window.history.go(-1);</script>') > -1:
			return False
		else:
			soup = bs(result, 'lxml')
			getValue = lambda selector: soup.select(selector)[0].attrs['value']
			data = {}
			data['name'] = getValue('#sname')
			data['ID'] = getValue('#sid').upper()
			data['class'] = getValue('#cla_id').replace('(', '（').replace(')', '）') + '班'
			data['student_ID'] = getValue('#ksh')
			return data
	if isinstance(id18, list):
		result = []
		for i in id18:
			result.append(Get(i))
		return result
